...
-- here in cuontry we have multiple country names too, now we need separate distinct names for each country separated by commas
select 
unnest(string_to_array(country,',')) as Country_name
from netflix;
--by this above clause we can separate country names and can get separate countries
...
select *, split_part(duration,' ',1)::numeric as seasons -- to split have to use column name, then inside of '' have to give space then
--have to mention which part we want to display, as we want our 1st part to be displayed here so we gave 1 as numeric number to select the first part
from netflix
where type = 'TV Show' and split_part(duration,' ',1)::numeric > '5'

--9. Count the number of content items in each genre
string_to_array(listed_in,',') -- to turn that full sentence into array
unnest(string_to_array(listed_in,',')) -- to turn this array into different different separate values
